...
bool install_pkg(const std::string &pkg, const std::string &pref_path) {
	...
	for (uint32_t i = 0; i < byte_swap(pkg_header.file_count); i++) {
		...
		if (fs::file_size(pkg) < byte_swap(pkg_header.data_offset) + byte_swap(entry.name_offset) + byte_swap(entry.name_size) ||
			fs::file_size(pkg) < byte_swap(pkg_header.data_offset) + byte_swap(entry.data_offset) + byte_swap(entry.data_size)) {
